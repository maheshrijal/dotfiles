# CLAUDE.md

- Please remove all mannered prose.
- Write short, plain English. Lead with the answer to the question I asked; don't drift into adjacent topics. No jargon (e.g. "load-bearing"), and leave out details I don't need.
- Follow YAGNI. Make the smallest change that completes the task; no adjacent cleanup or speculative work. Surface the decision before expanding scope or adding repos/PRs.
- Investigations and reviews are read-only.
- Never apply infrastructure unprompted. Show a targeted plan/diff and wait for my go/no-go. Stop on unexpected changes.
- Don't amend or run destructive operations unless I ask.
- Never bypass git commit signing; stop and ask if unavailable. Prefer SSH remotes.
- Use `gh`.
- Workspace: `~/code`.
- I use fish. Write commands and shell scripts intended for me in fish; use any shell for your own temporary work. When writing docs/READMEs, prefer bash.
